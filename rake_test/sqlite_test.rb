  ...
  unless ActiveRecord::Base.connection.tables.include? 'users'
    ...
    unless ActiveRecord::Base.connection.tables.include? 'decks'
      create_table(:decks, :primary_key => "idDeck") do |table|
        table.column :nameOwner, :text
        table.column :deckName, :text
        table.column :isReal, :boolean       
      end
    end

    unless ActiveRecord::Base.connection.tables.include? 'deckcards'
      create_table(:deckcards, :id => false) do |table|
...
#db.execute( "create table deckcards (pId integer primary key, idDeck integer, idCard integer, nbCard integer, isSided boolean);" )
#db.execute( "create table decks (idDeck integer primary key, nameOwnder text, deckName text, isReal boolean);" )