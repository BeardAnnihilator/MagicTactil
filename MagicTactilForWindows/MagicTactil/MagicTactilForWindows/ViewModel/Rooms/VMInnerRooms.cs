...
namespace MagicTactilForWindows.ViewModel
{
    ...
    public class VMInnerRooms : APage
    {
        #region attribute
        /*
         * link to the server
             ...
             if (ViewModelView != null)
             {
                 switch (page)
                 {
                     ...
                     case "CreateRoom":
                         ...
                         break;
                     case "Room":
                         ((VMRoom)ViewModels.ElementAt((int)InnerRoomView.ROOM)).refreshPlayerList();
                         ViewModelView.MoveCurrentToPosition((int)InnerRoomView.ROOM);
                         break;
                     case "Game":
                         ((VMGame)ViewModels.ElementAt((int)InnerRoomView.GAME)).loadDeck();
                         ViewModelView.MoveCurrentToPosition((int)InnerRoomView.GAME);
